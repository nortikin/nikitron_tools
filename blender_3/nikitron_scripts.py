...
my_str_classes = [
                ...
                'Curves_section', 'verticesNum_separator', 'shift_verts',
...
def maxim():
    ...
    if len(bpy.context.selected_objects) >= 2:
        o1,o2 = bpy.context.selected_objects[:2]   
        if o1.type == 'MESH' and o2.type == 'MESH':
    ...
    def calcVolume(self):
        ...
        for o in objs:
            ...
            if o.type == 'MESH':
                ...
                volumes[on] = [dn,vol]
                volume += vol
        bpy.data.objects[on].select_set(True)
        ...
        if obj[0].type == 'CURVE':
            ...
            for o in obj:
                ...
                if self.bezier:
                    ...
                    bpy.context.view_layer.objects.active = bpy.data.objects[nam]
                    bpy.ops.object.mode_set(mode='EDIT', toggle=False)
...
class ObjectNames (bpy.types.Operator):
    ...
    def run(self, origin,text,length):
       # Create and name TextCurve object
        bpy.ops.object.text_add(radius=1,align='WORLD',
...
class Connect2Meshes (bpy.types.Operator):
    ...
    def hooking_action(self, ob, nam, points, verts_of_object):
        ...
        bpy.data.scenes[bpy.context.scene.name].objects[ob.name].select_set(True)
        bpy.data.scenes[bpy.context.scene.name].objects[nam].select_set(True)
        bpy.context.view_layer.objects.active  = bpy.data.objects[nam]
        # deselect vertices
        bpy.ops.object.editmode_toggle()
...
class DeleteOrientation (bpy.types.Operator):
    ...
    def execute(self, context):
        bpy.data.scenes[bpy.context.scene.name].transform_orientation_slots[0]
        orients = bpy.data.scenes[bpy.context.scene.name].transform_orientation_slots
        for o in orients:
...
class BooleratorIntersection (bpy.types.Operator):
    ...
    def interinsects(self, ob1, ob0):
        ...
        bpy.ops.object.select_all(action='DESELECT')
        bpy.data.objects[name1].select_set(True)
        bpy.data.objects[name0].select_set(True)
        bpy.context.scene.objects.active = bpy.data.objects[name1]
...
class NT_PT_NikitronPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if main.nt_main_panel:
            ...
            row.operator("object.nt_delete_orientation",icon="ORIENTATION_NORMAL", text=sv_lang['DeleteOrientation'])
            ...
            if main.nt_additional_panel:
                if context.selected_objects:
                    ...
                    if context.selected_objects[0].type == 'MESH':
                        ...
                        row = col.row(align=True)
                        row.scale_y=1.1
                        row.prop(bpy.context.scene, "nt_shift_verts", text=sv_lang['shift_verts'])
                        row.prop(bpy.context.scene, "nt_hook_or_not", text=sv_lang['hook'])
                        row.label(text=sv_lang['maxvers'] + ' ' + str(maxim()))

my_classes = [
                CurvesTo3D, CurvesTo2D, NT_PT_NikitronPanel, ObjectNames,
                VerticesNumbers3D, Connect2Meshes, MaterialToObjectAll,
                MaterialToDataAll, BoundingBox, SpreadObjects,
                DeleteOrientation, SeparatorM, BooleratorRandom,
                BooleratorTranslation, BooleratorIntersection,
