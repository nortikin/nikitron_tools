...
class MP_PlaySIC(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if check[0]:
            ...
            check[0] = False
        bpy.types.WindowManager.mp_f = aud.Sound(context.scene.mp_playlist[context.window_manager.mp_index].playlist)
...
class VIEW3D_PT_Musicplayer(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not context.window_manager.mp_playing:
            ...
            sto = row_butt.operator("sound.stop", text="STOP ", icon='SNAP_FACE')
            sto.cicle_off = True
        if bpy.context.scene.mp_playlist_names.__len__():
